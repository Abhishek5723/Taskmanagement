 ...
 setCurrentEditedItem((prev)=>{
  ...
  return (
    ...
    <div className='todo-wrapper'>
       ...
       if(currentEdit===index) 
       {
        return(
          <div className='edit-wrapper' key={index}>
            ...
            <textarea placeholder='Updated Title' rows={4}   onChange={(e)=>handleUpdateDescription(e.target.value)} value={currentEditedItem.description}></textarea>
      ...
      <div>
        <AiOutlineDelete className='icon' onClick={()=>handleDeleteCompletedTodo(index)} title="Complete?"/>
        <BsCheckLg className='check-icon' onClick={()=>handleComplete(index)} title="Edit?"/>
      </div>
      </div>
        );
      })}
